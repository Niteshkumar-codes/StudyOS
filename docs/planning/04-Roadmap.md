...
- Playwright tests to cover mission-critical user paths:
  1. *Registration $\rightarrow$ OTP $\rightarrow$ Onboarding $\rightarrow$ Dashboard*
  2. *Start Timer $\rightarrow$ Complete Session $\rightarrow$ Validate Study Log*
  3. *Save Note $\rightarrow$ Create Flashcard $\rightarrow$ Verify card in revision stack*
